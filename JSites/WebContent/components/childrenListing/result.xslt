...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="/">
		...
		<xsl:if test="$state = 'WRK'">
			<c:include src="cocoon://components/childrenListing/view?cid={$cid}&amp;pid={$pid}&amp;time=strafuturo&amp;editing=true"/>	
		</xsl:if>
		<xsl:if test="$state = 'PND'">
			<c:include src="cocoon://components/childrenListing/view?cid={$cid}&amp;pid={$pid}&amp;time=futuro&amp;editing=true"/>	
		</xsl:if>
