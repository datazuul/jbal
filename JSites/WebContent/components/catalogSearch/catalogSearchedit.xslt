...
function move(elid,p) {
	...
	var mmerge1=document.getElementById("merge"+elid);
	...
	var mmerge2=document.getElementById("merge"+elidn);
...
function validate() {
	...
	s.value=o;
					...
					<input type="hidden" id="neworder" name="neworder" value="" />
				 	...
				 	<table class="catedit">
				 		<thead>
				 			<th></th><th>Order by</th><th>Search</th><th>List</th><th>Merge</th><th>Name</th><th>Label</th>
				 		</thead>
					 	...
					 	<xsl:for-each select="search">
					 		<tr>
						 		...
						 		<td>
						 			<xsl:variable name="na"><xsl:value-of select="@name" /></xsl:variable>
							 		<input type="checkbox" name="merge-{@name}" value="true" id="merge{position()}">
							 			<xsl:if test="string-length(//merge[@name=$na]/@checked) != 0">
							 				<xsl:attribute name="checked">checked</xsl:attribute>
							 			</xsl:if>
							 			<xsl:if test="@name = 'ANY' ">
							 				<xsl:attribute name="disabled">disabled</xsl:attribute>
