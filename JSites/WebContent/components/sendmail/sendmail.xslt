...
<xsl:stylesheet version="1.0" xmlns:sql="http://apache.org/cocoon/SQL/2.0"
	...
	<xsl:template match="root">
		<content>
			...
			<xsl:variable name="urlrequest">http://<xsl:value-of select="translate(h:request/h:requestHeaders/h:header[@name='host'],$uppercase,$lowercase)" /><xsl:value-of select="translate($pagerequest,$uppercase,$lowercase)" /></xsl:variable>
			<xsl:variable name="referer"><xsl:value-of select="translate(h:request/h:requestHeaders/h:header[@name='referer']/text(),$uppercase,$lowercase)" /></xsl:variable>
			<xsl:if test="not(contains($referer,$urlrequest))">
				<xsl:value-of select="//welcome" />
			</xsl:if>
			
			<xsl:if test="contains($referer,$urlrequest)">
				<xsl:variable name="r">
