...
//c.dayHeaders = new Array("Do","Lu","Ma","Me","Gi","Ve","Sa");
...
function CP_getCalendar() {
	...
	if (this.type == "WINDOW") {
		result += "<HTML><HEAD><TITLE>Inf.Or. srl - Seleziona Data</TITLE>"+this.getStyles()+"</HEAD><BODY MARGINWIDTH=0 MARGINHEIGHT=0 TOPMARGIN=0 RIGHTMARGIN=0 LEFTMARGIN=0>\n";
	...
	if (this.displayType=="date" || this.displayType=="week-end") {
		...
		result += '<TR>\n';
                var coloreDomenica;
		for (var j=0; j<7; j++) {
                        if (j == 0){coloreDomenica = 'red';}else{coloreDomenica = 'black';}
			result += '<TD CLASS="'+this.cssPrefix+'cpDayColumnHeader" WIDTH="14%"><FONT COLOR="'+coloreDomenica+'"><SPAN CLASS="'+this.cssPrefix+'cpDayColumnHeader">'+this.dayHeaders[(this.weekStartDay+j)%7]+'</FONT></TD>\n';
			}
		result += '</TR>\n';
