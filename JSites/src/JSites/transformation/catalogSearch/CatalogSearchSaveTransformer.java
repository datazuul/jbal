...
public class CatalogSearchSaveTransformer extends MyAbstractPageTransformer {
	...
	public void startElement(String uri, String loc, String raw, Attributes a) throws SAXException {
		if(loc.equals("catalogSearch")) {
			...
			mergeChannel=new Hashtable<String,Channel>();
		...
		else if(loc.equals("merge")){
			String name=a.getValue("name");
			String desc=a.getValue("desc");
			if(name!=null)
				mergeChannel.put(name, new Channel(name,desc,""));
		}
		else {
	...
	public void endElement(String uri, String loc, String raw) throws SAXException {
		if(loc.equals("catalogSearch")) {
			...
			for(int k=0;chr!=null && k<chr.length;k++) {
				...
				if(!mergeChannel.containsKey(tc)) mergeChannel.put(tc, new Channel(tc,tc,""));
			}
...
//			}
			...
			while(i.hasMoreElements()) {
				String chn=i.nextElement();
				Channel ch=mergeChannel.get(chn);
				String t=request.getParameter("merge-"+ch.getName());
				String desc=request.getParameter("name-"+ch.getName());
				if(desc==null) desc=ch.getName();
				ch.setDesc(desc);
				if(t!=null && t.length()>0) ch.setChecked("true");
				sendElement("merge",ch);
			}
		...
		else if(loc.equals("search") || loc.equals("list") || loc.equals("merge")) {
			// do nothing
		}
		else {
