...
//		}
	...
	public long insertRecord(Radice stemmer, RecordInterface record) throws Exception {
	...
	public void insertRecord(Radice stemmer, RecordInterface record, long jid) throws Exception {
		ParoleSpooler paroleSpooler=new ParoleSpooler(new Connection[] {conn},catalog,1,null,stemmer,System.out);
		DbGateway dbg=DbGateway.getInstance("mysql", null);
		dbg.inserisciNotizia(conn, catalog, stemmer, paroleSpooler, record, jid);
	}

	public void importRecords(InputStream in, String format, boolean background,
...
//		}
		...
		DataImporter dataimporter=new DataImporter(in,format,null, conns, catalog, true, cache, console,console); //,t);
		if(background) {
			dataimporter.start();
		}
		else {
