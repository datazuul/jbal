...
public class NewItemCardinality implements org.jopac2.engine.ItemCardinality {
	...
	private String search_anagrafe_parole_limit="500";
...
/*	private BitSet doRicercaBitArray(Connection conn, long id_lcp) {
	...
	*/
	...
	public BitSet doRicercaBitArrayAny(Connection conn, String catalog, boolean useStemmer) throws SQLException {
		...
		//try {
			String sql;
			String anagrafe_parole_channel="parola";
			if(useStemmer) anagrafe_parole_channel="stemma";
			
			sql = "SELECT DISTINCT lcpn.id_notizia "
					+ "     FROM je_"+catalog+"_l_classi_parole_notizie lcpn," 
					+ "		     je_"+catalog+"_l_classi_parole lcp, " 
					+"           (select * from je_"+catalog+"_anagrafe_parole where "+anagrafe_parole_channel+" like ? limit "+search_anagrafe_parole_limit+") a "
					+ "    WHERE " 
					+ "          lcp.id_parola=a.id "
...
//		}
	...
	public BitSet doRicercaBitArrayJolly(Connection conn, String catalog, boolean useStemmer) throws SQLException {
		...
		//try {
			String sql;
			String anagrafe_parola_channel="parola";
			if(useStemmer) anagrafe_parola_channel="stemma";
			
			sql= "SELECT DISTINCT lcpn.id_notizia "
				+ "     FROM je_"+catalog+"_l_classi_parole lcp," // QUESTA TABELLA DEVE ESSERE LA PRIMA
			    + "          je_"+catalog+"_l_classi_parole_notizie lcpn," 
				+ "          (select * from je_"+catalog+"_anagrafe_parole where "+anagrafe_parola_channel+" like ? limit "+search_anagrafe_parole_limit+") a "
				+ "    WHERE lcp.id_classe = ?"
