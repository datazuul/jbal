...
public class StemmerItv2 extends Radice {
     ...
     *    parola
     * se la variabile mod Ã¨ true viene controllato se ci sono anche i suffissi
     *    ando endo ar er ir e cosÃ¬ vengono modificati e si esce
    ...
    private void step1() {
        ...
        if (k>7){ //7 lettere
            ...
            if ((ends("abilitï¿½"))&&(k-7>=r2)) { k = k - 7; modifica = false; return;}
        ...
        if (k>5){ //5 lettere
            ...
            if ((ends("icitï¿½"))&&(k-5>=r2)) { k = k - 5; modifica = false; return;}
            if ((ends("ivitï¿½"))&&(k-5>=r2)) { k = k - 5; modifica = false; return;}
        ...
        if (k>4){ //4 lettere
            ...
            if (ends("istï¿½")&&(k-4>=r2)) {k = k - 4; modifica = false; return;}
            if (ends("istï¿½")&&(k-4>=r2)) {k = k - 4; modifica = false; return;}
            if (ends("istï¿½")&&(k-4>=r2)) {k = k - 4; modifica = false; return;}
        ...
        if(k>3) { //3 lettere
            ...
            if (ends("itï¿½")&&(k-3>=r2)) { k = k - 3; modifica = false; return;}
     ...
     * Viene eseguita se e solo se la variabile modifica ï¿½ true.
    ...
    private void step2() {
        ...
        if(k>3) { //3 lettere
            ...
            if (ends("erï¿½")&&(k-3>=rv)) { k = k - 3; return;}
            ...
            if (ends("irï¿½")&&(k-3>=rv)) { k = k - 3; return;}
     ...
     * toglie tutte le vocali finali fatto eccezione per la u che se Ã¨ presente
     * fa parte della radice.
     * e toglie la i se Ã¨ seguita da una vocale
     */
    private void step3a() {
        ...
        if ((k-1)>=rv){
            switch (b[k]){
                ...
                case '': // e accentata
                case '“': // i accentata
                case '˜': { k = k-1; precedente = true; break; } // ˜ accentata
                //case '': { k = k-1; break; } // u accentata
                default: return;
            }
        }
     ...
     * che decino se una determinato suffisso puï¿½ essere eliminato o no
     ...
     * che decide se una determinato suffisso puï¿½ essere eliminato o no.
     * Se la seconda lettere ï¿½ una consonate rv diventa la prima vocale successiva.
     ...
     * Se perï¿½ la parola ï¿½ piï¿½ corta rv rimane a zero.
     */
    private void puntatorerv(){
        ...
        // if dove la seconda lettera ï¿½ una consonante
        if (cons(1)) {
            indice = 1;
            //va avanti fino a quando non trova una vocale
            while(true){
        ...
        } // fine if dove la seconda lettera ï¿½ una consonante
    ...
    /* Determina se all'iesima posizone c'ï¿½ una consonante o una vocale
     * torna true se ï¿½ una consonante
     * e false se ï¿½ una vocale
     */
