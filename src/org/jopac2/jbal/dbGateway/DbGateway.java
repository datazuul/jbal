...
import org.jopac2.jbal.importers.LoadClasses;
...
public final class DbGateway {
	...
	public static Hashtable<String, Long> getTipiNotizie(Connection conn) {
	     Hashtable<String,Long> tipi = new Hashtable<String,Long>();
	     if(conn!=null) {
		     try {
			     Statement stmt=conn.createStatement();
			     ResultSet rs=stmt.executeQuery("SELECT * from tipi_notizie");
			     while (rs.next()) {
			       tipi.put(new String(rs.getString(2)), new Long(rs.getLong(1)));
			     }
			    rs.close();
			    stmt.close();
		     }
	...
	public static Hashtable<String,String> getClassi() {
		Hashtable<String,String> classi=new Hashtable<String,String>();
	...
	public static Hashtable<String,String> getValori() {
		Hashtable<String,String> valori=new Hashtable<String,String>();
...
//        execute("drop table if exists temp_lcpn");
	...
	public static void insertClassItem(Connection conn, ClassItem currentItem) {
        ...
        long currentElementID=DbGateway.getElementNameID(conn, currentItem.getElementName());
...
//            boolean rep=false;
            ...
            if(rs.next()) {
	            ...
	            ma=RecordFactory.buildRecord(rs.getLong("id"),org.jopac2.utils.ZipUnzip.decompressString(notizia.getBytes(1, (int)notizia.length())),tipo,0);
...
/*    rs=stmt.executeQuery("SELECT Max(data_element.id) AS MaxOfid FROM data_element;");
	...
	private static long insertTableNotizie(Connection conn, RecordInterface notizia, long idTipo) throws SQLException {
        ...
        try {
        	...
        	preparedNotizia.setBytes(4, org.jopac2.utils.ZipUnzip.compressString(notizia.toString()));
...
//			        System.out.println("WARNING! id_notizia:"+Long.toString(id_notizia)+" / tag: "+tag+" / de: "+id_cl_dettaglio+" ignored");
	...
	public static String getFilter(Connection conn, String filetype) {
		String ret=null;
		try {
			String query="select tag " +
					"from classi, classi_dettaglio, tipi_notizie " +
					"where classi_dettaglio.id_classe=classi.id and " +
						"classi.nome='filter' and " +
						"classi_dettaglio.id_tipo=tipi_notizie.id and " +
						"tipi_notizie.nome='"+filetype+"'";
	        Statement stmt=conn.createStatement();
	        ResultSet rs=stmt.executeQuery(query);
	        
	        if(rs.next()) ret=rs.getString("tag");
	        rs.close();
	        stmt.close();
	      }
	      catch (Exception e) {
